...
import d3Sankey from '../libs/sankeyCircular';
import {
  DefaultLink,
  DefaultNode,
  SankeyAlignFunction,
...
export const computeNodeAndLinks = <N extends DefaultNode, L extends DefaultLink>({
  ...
  const data = JSON.parse(JSON.stringify(_data)) as unknown as {
...
export const useSankey = <N extends DefaultNode, L extends DefaultLink>({
  ...
  const getLabel = usePropertyAccessor<Omit<SankeyNodeDatum<N, L>, 'color' | 'label'>, string>(label);
